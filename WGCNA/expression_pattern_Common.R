...
Common_pH_pos <- ggplot(dat1,aes(pH,Nb)) +
  ...
  geom_jitter(aes(colour=Site),size=0.5, width = 0.003, alpha=0.5) + 
...
Common_Length_pos <- ggplot(dat1,aes(Length,Nb)) +
  ...
  geom_jitter(aes(colour=Site),size=0.5, width = 0.03, alpha=0.5) + 
  #  stat_summary(fun=median, geom="line", aes(group=1))+
  #  geom_line(aes(colour=Site)) +
  theme_bw()+ #背景变为白色
  theme(axis.text.x=element_text(colour="black",family="Arial",size=13), #设置x轴刻度标签的字体显示倾斜角度为15度，并向下调整1(hjust = 1)，字体簇为Arial大小为20
...
Common_Salinity_pos <- ggplot(dat1,aes(Salinity,Nb)) +
  ...
  geom_jitter(aes(colour=Site),size=0.7, width = 0.05, alpha=0.5) + 
  #  stat_summary(fun=median, geom="line", aes(group=1))+
  #  geom_line(aes(colour=Site)) +
  theme_bw()+ #背景变为白色
  theme(axis.text.x=element_text(colour="black",family="Arial",size=13), #设置x轴刻度标签的字体显示倾斜角度为15度，并向下调整1(hjust = 1)，字体簇为Arial大小为20
