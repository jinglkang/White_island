...
names(dat1)
Common_pH_pos <- ggplot(dat1,aes(pH,Nb)) +
  geom_boxplot(aes(group=pH,colour=Site), coef = 6, outlier.shape=NA, width = 0.02) +
  geom_jitter(aes(colour=Site),size=0.5,width = 0.01) + 
...
names(dat1)
Common_pH_neg <- ggplot(dat1,aes(pH,Nb)) +
  geom_boxplot(aes(group=pH,colour=Site), coef = 6, outlier.shape=NA, width = 0.02) +
  geom_jitter(aes(colour=Site),size=0.5,width = 0.01) + 
  #  stat_summary(fun=median, geom="line", aes(group=1))+
  #  geom_line(aes(colour=Site)) +
...
names(dat1)
Common_Length_pos <- ggplot(dat1,aes(Length,Nb)) +
  geom_boxplot(aes(group=Length,colour=Site), coef = 6, outlier.shape=NA, width = 0.15) +
  geom_jitter(aes(colour=Site),size=0.5,width = 0.05) + 
  #  stat_summary(fun=median, geom="line", aes(group=1))+
  #  geom_line(aes(colour=Site)) +
  theme_bw()+ #背景变为白色
  theme(axis.text.x=element_text(colour="black",family="Times",size=13), #设置x轴刻度标签的字体显示倾斜角度为15度，并向下调整1(hjust = 1)，字体簇为Times大小为20
        ...
        panel.grid.major = element_blank(),   #不显示网格线
        panel.grid.minor = element_blank())+  #不显示网格线
  ylab("Expression Ratio")+xlab("Length")+ #设置x轴和y轴的标题
  theme(legend.position = "top",legend.title = element_blank()) +
  scale_color_aaas() + geom_smooth(method=lm, fullrange=TRUE, colour="#8081807F", se=F) +
...
names(dat1)
Common_Length_neg <- ggplot(dat1,aes(Length,Nb)) +
  geom_boxplot(aes(group=Length,colour=Site), coef = 6, outlier.shape=NA, width = 0.15) +
  geom_jitter(aes(colour=Site),size=0.5,width = 0.05) + 
  #  stat_summary(fun=median, geom="line", aes(group=1))+
  #  geom_line(aes(colour=Site)) +
  ...
  theme(legend.position = "top",legend.title = element_blank()) +
  scale_color_aaas() + geom_smooth(method=lm, fullrange=TRUE, colour="#8081807F", se=F) +
  annotate(x=5.2, y=0.15, label=paste("Common: ",length(unique(dat1$Gene)), " genes\n","R = ", round(cor(dat1$Length, dat1$Nb),2),
                                      " (p value = ", signif(cor.test(dat1$Length, dat1$Nb)$p.value, 3),")", sep = ""), 
           geom="text", size=5, colour="#1B1919FF", family="Times")
...
names(dat1)
Common_Salinity_pos <- ggplot(dat1,aes(Salinity,Nb)) +
  ...
  theme(axis.text.x=element_text(colour="black",family="Times",size=13), #设置x轴刻度标签的字体显示倾斜角度为15度，并向下调整1(hjust = 1)，字体簇为Times大小为20
        ...
        panel.grid.major = element_blank(),   #不显示网格线
        panel.grid.minor = element_blank())+  #不显示网格线
  ylab("Expression Ratio")+xlab("Salinity")+ #设置x轴和y轴的标题
  theme(legend.position = "top",legend.title = element_blank()) +
  scale_color_aaas() + geom_smooth(method=lm, fullrange=TRUE, colour="#8081807F", se=F) +
  annotate(x=33, y=0.15, label=paste("Common: ",length(unique(dat1$Gene)), " genes\n","R = ", round(cor(dat1$Salinity, dat1$Nb),2),
...
names(dat1)
Common_Salinity_neg <- ggplot(dat1,aes(Salinity,Nb)) +
  geom_boxplot(aes(group=Salinity,colour=Site), coef = 6, outlier.shape=NA, width = 0.15) +
  geom_jitter(aes(colour=Site),size=0.5,width = 0.05) + 
  #  stat_summary(fun=median, geom="line", aes(group=1))+
  #  geom_line(aes(colour=Site)) +
  ...
  theme(legend.position = "top",legend.title = element_blank()) +
  scale_color_aaas() + geom_smooth(method=lm, fullrange=TRUE, colour="#8081807F", se=F) +
  annotate(x=34, y=0.2, label=paste("Common: ",length(unique(dat1$Gene)), " genes\n","R = ", round(cor(dat1$Salinity, dat1$Nb),2),
                                     " (p value = ", signif(cor.test(dat1$Salinity, dat1$Nb)$p.value, 3),")", sep = ""), 
           geom="text", size=5, colour="#1B1919FF", family="Times")
